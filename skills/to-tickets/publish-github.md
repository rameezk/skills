...
- **Status** - a label. Apply `ready-for-agent` or `ready-for-human` to match the quiz, ensuring each exists first with the idempotent `gh label create ready-for-agent --force` / `gh label create ready-for-human --force`. `done` is recorded by *closing* the issue - done by the dispatch step, never here (so a PR's `Closes #NN` can do it).
